    ...
    <div className="code-window glass-panel min-w-0 overflow-hidden rounded-[28px] border border-white/12 bg-[#06101d]/80 shadow-glow">
      <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-4 sm:px-5">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-slate-400 sm:text-sm">main.aero</span>
        </div>
        <button
          type="button"
          onClick={handleCopy}
          className="shrink-0 rounded-full border border-cyan/20 bg-cyan/10 px-3 py-1 text-xs font-medium text-cyan transition hover:border-cyan/40 hover:bg-cyan/15"
        >
          {copied ? "Copied" : "Copy"}
        </button>
      </div>
